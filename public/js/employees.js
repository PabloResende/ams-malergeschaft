...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const closeModalBtns = document.querySelectorAll('#closeEmployeeModal, #cancelEmployeeModal');
    ...
    const filterBtns = ['adminFilterToday', 'adminFilterWeek', 'adminFilterMonth', 'adminFilterAll'];
    ...
    function activateCreateTab(tabName) {
        ...
        const allPanels = document.querySelectorAll('#employeeCreateModal .tab-panel');
    ...
    function activateDetailsTab(tabName) {
        detailsTabBtns.forEach(btn => {
            ...
            btn.classList.add('text-gray-600', 'hover:text-gray-800', 'border-transparent');
        });

        const activeBtn = document.querySelector(`[data-tab="${tabName}"]`);
        if (activeBtn && activeBtn.classList.contains('tab-btn')) {
            activeBtn.classList.add('bg-blue-100', 'text-blue-600', 'border-blue-600');
            activeBtn.classList.remove('text-gray-600', 'hover:text-gray-800', 'border-transparent');
        }

        const allPanels = document.querySelectorAll('#employeeDetailsModal .tab-panel');
        allPanels.forEach(panel => panel.classList.add('hidden'));
    ...
    async function loadEmployeeDetails(employeeId) {
        try {
            const response = await fetch(`${window.baseUrl}/employees/get?id=${employeeId}`);
    ...
    function fillEmployeeDetails(employee) {
        ...
        document.getElementById('detailsLoginUserId').value = employee.user_id || '';
        ...
        const documents = [
            'passport', 'permissionphotofront', 'permissionphotoback',
            'healthcardfront', 'healthcardback', 'bankcardfront',
            'bankcardback', 'marriagecertificate'
        ];

        documents.forEach(doc => {
            const img = document.getElementById(`view${doc.charAt(0).toUpperCase() + doc.slice(1)}`);
            const link = document.getElementById(`link${doc.charAt(0).toUpperCase() + doc.slice(1)}`);
    ...
    function fillTransactions(transactions) {
        ...
        tbody.innerHTML = transactions.map(tx => `
            <tr class="border-t">
                <td class="p-2 text-sm">${new Date(tx.date).toLocaleDateString('pt-BR')}</td>
                <td class="p-2 text-sm">${tx.type || 'N/A'}</td>
                <td class="p-2 text-sm text-right">CHF ${parseFloat(tx.amount || 0).toFixed(2)}</td>
            </tr>
        `).join('');
    }
    ...
    async function loadEmployeeHours(employeeId) {
        ...
        try {
            const response = await fetch(`${window.baseUrl}/api/employees/hours/${employeeId}`);
                ...
                tbody.innerHTML = `
                    <tr>
                        <td colspan="4" class="p-4 text-center text-red-500">
                            ${window.langText.error_loading_hours || 'Erro ao carregar horas'}
                        </td>
                    </tr>`;
            }
        }
    ...
    function renderFilteredHours(filter = 'all') {
        ...
        if (!window.allHoursData || !window.allHoursData.length) {
            if (tbody) {
                tbody.innerHTML = `
                    <tr>
                        <td colspan="4" class="p-4 text-center text-gray-500">
                            ${window.langText.no_hours_registered || 'Nenhuma hora cadastrada'}
                        </td>
                    </tr>`;
            }
        ...
        const totalHours = filteredEntries.reduce((sum, entry) => sum + parseFloat(entry.total_hours || 0), 0);
        ...
        if (!filteredEntries.length) {
            tbody.innerHTML = `
                <tr>
                    <td colspan="4" class="p-4 text-center text-gray-500">
                        ${window.langText.no_hours_found || 'Nenhum registro encontrado para este período'}
                    </td>
                </tr>`;
            return;
        }
    ...
    function updateFilterButtons(activeFilter) {
        const filters = ['adminFilterToday', 'adminFilterWeek', 'adminFilterMonth', 'adminFilterAll'];
        const filterMap = {
            'adminFilterToday': 'today',
            'adminFilterWeek': 'week', 
            'adminFilterMonth': 'month',
        ...
        filters.forEach(filterId => {
            ...
            if (filterMap[filterId] === activeFilter) {
                btn.className = 'px-4 py-2 border rounded-lg text-sm bg-blue-50 text-blue-600 border-blue-300';
            } else {
