...
class TransactionModel
{
    ...
    public static function getAll(array $f=[]): array {
        $sql = "
          SELECT ft.*, fc.name AS category_name, d.*
    ...
    public static function getSummary(string $start,string $end): array {
        ...
        return ['income'=>$totIn,'expense'=>$totEx,'net'=>$totIn-$totEx];
    }
    ...
    public static function store(array $data,array $attachments=[],$debtData=null): int {
        $pdo=self::connect();
        $stmt=$pdo->prepare("
    ...
    public static function update(int $id,array $data,array $attachments=[],$debtData=null): void {
        $pdo=self::connect();
        $pdo->prepare("
