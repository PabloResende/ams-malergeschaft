...
(function () {
  ...
  const timeTrackingEmployeeId = document.getElementById(
    "timeTrackingEmployeeId"
  );
  ...
  const employeeModalTotalHours = document.getElementById(
    "employeeModalTotalHours"
  );
      ...
      fetch(`${window.baseUrl}/employees/get?id=${id}`)
        ...
        .then((data) => {
          ...
          document.getElementById("modalEmployeeEmail").textContent =
            emp.email || "N/A";
          document.getElementById("modalEmployeeFunction").textContent =
            emp.function || "N/A";
          document.getElementById("modalEmployeePhone").textContent =
            emp.phone || "N/A";
          document.getElementById("modalEmployeeAddress").textContent =
            emp.address || "N/A";
          ...
          if (form) {
            ...
            form.querySelector('[name="birth_date"]').value =
              emp.birth_date || "";
            form.querySelector('[name="nationality"]').value =
              emp.nationality || "";
  ...
  async function handleAdminTimeEntrySubmit(e) {
    ...
    try {
      ...
      const response = await fetch(
        `${window.baseUrl}/api/work_logs/admin_time_entry`,
        {
          method: "POST",
          headers: {
  ...
  async function loadEmployeeHours(employeeId, filter = "all") {
    ...
    try {
      console.log(
        `🔄 Carregando horas do funcionário ${employeeId} com filtro ${filter}...`
      );
      ...
      const response = await fetch(
        `${window.baseUrl}/api/employees/${employeeId}/hours?filter=${filter}`
      );
      ...
      if (employeeModalTotalHours) {
        employeeModalTotalHours.textContent = `${parseFloat(
          result.total_hours || 0
        ).toFixed(1)}h`;
      }

      if (result.entries && result.entries.length > 0) {
        employeeHoursList.innerHTML = result.entries
          .map(
            (entry) => `
        ...
        <div class="p-8 text-center">
          <div class="text-sm text-red-500">
            ${
              ...
              "Erro ao carregar registros de horas"
            }
          </div>
        </div>
      `;
    ...
    employeeForm.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        if (response.ok && result.success) {
          ...
          showNotification(
            result.message || "Erro ao salvar funcionário",
            "error"
          );
        }
      } catch (error) {
  ...
  async function loadEmployeeData() {
    ...
    for (const element of employeeRows) {
      ...
      if (employeeId) {
        try {
          const response = await fetch(
            `${window.baseUrl}/api/employees/${employeeId}/hours-summary`
          );
          ...
          console.error(
            `Erro ao carregar horas do funcionário ${employeeId}:`,
            error
          );
  ...
  window.createEmployee = async function (formData) {
    try {
      ...
      if (result.success) {
        ...
        showNotification(
          result.message || "Erro ao criar funcionário",
          "error"
        );
        return false;
      }
  ...
  window.updateEmployee = async function (employeeId, formData) {
    try {
      const response = await fetch(
        `${window.baseUrl}/api/employees/${employeeId}/update`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (result.success) {
        ...
        showNotification(
          result.message || "Erro ao atualizar funcionário",
          "error"
        );
        return false;
      }
  ...
  window.deleteEmployee = async function (employeeId) {
    ...
    try {
      const response = await fetch(
        `${window.baseUrl}/api/employees/${employeeId}/delete`,
        {
          method: "POST",
        }
      );
      ...
      if (result.success) {
        ...
        showNotification(
          result.message || "Erro ao excluir funcionário",
          "error"
        );
        return false;
      }
