...
// Constrói array de flags para seleção de idioma
$flags = [
...
// Flag atual (ícone) para o idioma ativo
$currentFlag = isset($flags[$lang])
    ? '<img src="' . $flags[$lang]['flag'] . '" class="w-5 h-5" alt="' . htmlspecialchars($flags[$lang]['name']) . '">'
    : '🌎';

// Verifica se o usuário está logado
$isLoggedIn = isset($_SESSION['user']);
// Evita exibir header em páginas de login ou registro
$isLoginOrRegisterPage = str_contains($_SERVER['REQUEST_URI'], 'login')
...
<head>
  ...
  <style>
    ...
    .content-wrapper { padding-top: 60px; padding-bottom: 70px; }
    .bottom-nav { box-shadow: 0 -1px 5px rgba(0,0,0,0.1); border-top: 1px solid #e5e7eb; }
    .dropdown-menu { position: absolute; right: 0; margin-top: 0.5rem; width: 160px;
        ...
        <div class="relative">
          <button class="notification-btn relative bg-transparent focus:outline-none">
            ...
            <?php if (!empty($notifications)): ?>
              <span class="notification-dot absolute top-0 right-0 bg-red-600 text-white w-5 h-5 rounded-full flex items-center justify-center text-xs">
                <?= count($notifications) ?>
              </span>
            <?php endif; ?>
          </button>
