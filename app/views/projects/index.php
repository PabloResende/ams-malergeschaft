...
<div class="ml-56 pt-20 p-8 relative">
  ...
  <div id="projectModal"
    ...
    <div class="bg-white rounded-md p-8 w-90 max-h-[90vh] overflow-y-auto mt-10 relative">
      ...
      <h3 class="text-xl font-bold mb-4">
        <?= htmlspecialchars($langText['add_project'] ?? 'Add Project', ENT_QUOTES, 'UTF-8') ?>
      </h3>
      <form id="projectForm" action="<?= $baseUrl ?>/projects/store" method="POST">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label class="block text-gray-700">
              <?= htmlspecialchars($langText['name'] ?? 'Project Name', ENT_QUOTES, 'UTF-8') ?>
            </label>
            <input name="name" type="text" required class="w-full p-2 border rounded">
          </div>
          <div>
            <label class="block text-gray-700">
              <?= htmlspecialchars($langText['select_client'] ?? 'Select Client', ENT_QUOTES, 'UTF-8') ?>
            </label>
            <select name="client_id" class="w-full p-2 border rounded">
              <option value="">
                <?= htmlspecialchars($langText['select_client'] ?? 'Select a client', ENT_QUOTES, 'UTF-8') ?>
              </option>
          ...
          <div>
            <label class="block text-gray-700">
              <?= htmlspecialchars($langText['location'] ?? 'Location', ENT_QUOTES, 'UTF-8') ?>
            </label>
            <input name="location" type="text" class="w-full p-2 border rounded">
          </div>
          <div>
            <label class="block text-gray-700">
              <?= htmlspecialchars($langText['description'] ?? 'Description', ENT_QUOTES, 'UTF-8') ?>
            </label>
            <textarea name="description" class="w-full p-2 border rounded"></textarea>
          </div>
          <div>
            <label class="block text-gray-700">
              <?= htmlspecialchars($langText['start_date'] ?? 'Start Date', ENT_QUOTES, 'UTF-8') ?>
            </label>
            <input name="start_date" type="date" required class="w-full p-2 border rounded">
          </div>
          <div>
            <label class="block text-gray-700">
              <?= htmlspecialchars($langText['end_date'] ?? 'End Date', ENT_QUOTES, 'UTF-8') ?>
            </label>
        ...
        <div class="mb-4">
          <label class="block text-gray-700">
            <?= htmlspecialchars($langText['tasks'] ?? 'Tasks', ENT_QUOTES, 'UTF-8') ?>
          </label>
        ...
        <div class="mb-4">
          <label class="block text-gray-700">
            <?= htmlspecialchars($langText['employees'] ?? 'Employees', ENT_QUOTES, 'UTF-8') ?>
          </label>
          <div id="createEmployeesContainer"></div>
          <div class="flex mt-2">
            <select id="createEmployeeSelect" class="w-full p-2 border rounded">
              <option value="">
                <?= htmlspecialchars($langText['select_employee'] ?? 'Select an employee', ENT_QUOTES, 'UTF-8') ?>
              </option>
  ...
  <!-- Modal de Detalhes / Edição com Abas -->
      ...
      <form id="projectDetailsForm" action="<?= $baseUrl ?>/projects/update" method="POST">
          ...
          <ul class="flex space-x-6 border-b">
            ...
            <li>
              <button type="button"
                      ...
                      data-tab="tarefas">
                <?= htmlspecialchars($langText['tasks'] ?? 'Tarefas', ENT_QUOTES, 'UTF-8') ?>
              </button>
            </li>
            <li>
              <button type="button"
                ...
                <?= htmlspecialchars($langText['employees'] ?? 'Funcionários', ENT_QUOTES, 'UTF-8') ?>
              </button>
            </li>
            <li>
              <button type="button"
                      class="tab-btn pb-3 font-medium text-gray-600 hover:text-gray-800"
                      data-tab="inventario">
                <?= htmlspecialchars($langText['inventory'] ?? 'Inventário', ENT_QUOTES, 'UTF-8') ?>
              </button>
            </li>
          </ul>
        </nav>
        ...
        <div class="tab-content space-y-6">
          ...
          <div id="tab-geral" class="tab-panel">
            ...
            <div class="mb-4">
              <label class="block text-gray-700 font-semibold">
                <?= htmlspecialchars($langText['name'] ?? 'Name', ENT_QUOTES, 'UTF-8') ?>
              </label>
              <input name="name" type="text" id="detailsProjectName"
            ...
            <div class="mb-4">
              ...
              <textarea name="description" id="detailsProjectDescription"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input name="start_date" type="date" id="detailsProjectStartDate"
              ...
              <div>
                ...
                <input name="end_date" type="date" id="detailsProjectEndDate"
                       required class="w-full p-2 border rounded">
              </div>
            </div>
          </div>
          ...
          <div id="tab-inventario" class="tab-panel hidden">
            <label class="block text-gray-700 font-semibold mb-2">
              <?= htmlspecialchars($langText['inventory'] ?? 'Inventory', ENT_QUOTES, 'UTF-8') ?>
            </label>
