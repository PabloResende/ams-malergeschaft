  ...
  <div id="transactionModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-lg mx-4 overflow-hidden">
      ...
      <div class="flex border-b">
        ...
        <button id="tabDebtBtn"
                class="flex-1 text-center px-4 py-2 text-gray-500 hover:text-gray-700 hidden">
          Parcelamento
        </button>
      </div>
      ...
      <form id="transactionForm" method="POST" enctype="multipart/form-data"
        ...
        <div id="tabGeneral" class="space-y-4">
          ...
          <div>
            ...
            <select name="category" id="txCategorySelect" required
                    class="w-full border rounded p-2">
              <option value="" disabled selected>Selecione categoria</option>
              <?php foreach($categories as $c): ?>
                <option value="<?= $c['value'] ?>" data-assoc="<?= $c['assoc'] ?>">
                  <?= htmlspecialchars($c['name']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div>
            ...
            <input type="number" step="0.01" name="amount" id="txAmountInput" required
                   class="w-full border rounded p-2"/>
          </div>
          ...
          <div>
            ...
            <textarea name="description" id="txDescInput" rows="3"
        ...
        <div id="tabDebt" class="hidden space-y-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <label for="initialPaymentChk" class="font-medium">Entrada</label>
          </div>
          <div id="initialPaymentContainer" class="hidden">
            <label class="block mb-1 font-medium">Valor da Entrada</label>
            <input type="number" step="0.01" id="initialPaymentAmt"
                   name="initial_payment_amount" class="w-full border rounded p-2"/>
          </div>
          <div>
            <label class="block mb-1 font-medium">Número de Parcelas</label>
            <select id="installmentsSelect" name="installments_count"
                    class="w-full border rounded p-2">
              <option value="" disabled selected>Selecione</option>
              <?php for($i=1; $i<=12; $i++): ?>
        ...
        <div class="flex justify-end gap-2 pt-4 border-t">
          <button type="button" id="txCancelBtn"
                  class="px-4 py-2 border rounded hover:bg-gray-100">Cancelar</button>
          <button type="submit" id="txSaveBtn"
                  class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Salvar</button>
        </div>
      </form>

      <a id="txDeleteLink" href="#"
         class="absolute bottom-4 left-6 text-red-600 hover:underline hidden">
        Excluir
      </a>
    </div>
  </div>
