...
// 3) Carrega clients para o select de criação
$clients = $pdo
...
<div class="ml-56 pt-20 p-8 relative">
  ...
  <div id="projectsGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    <?php if (empty($projects)): ?>
      <p>
        <?= htmlspecialchars($langText['no_projects_available'] ?? 'No projects available.', ENT_QUOTES, 'UTF-8') ?>
      </p>
    <?php else: ?>
      <?php foreach ($projects as $project): ?>
        ...
        >
          ...
          <p class="text-sm text-gray-600 mb-2">
            <span class="font-semibold">
              <?= htmlspecialchars($langText['budget'] ?? 'Budget', ENT_QUOTES, 'UTF-8') ?>:
            </span>
    ...
    <div class="bg-white rounded-md p-8 w-90 max-h-[90vh] overflow-y-auto mt-10 relative">
      ...
      <form id="projectForm" action="<?= $baseUrl ?>/projects/store" method="POST">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label class="block text-gray-700">
              <?= htmlspecialchars($langText['status'] ?? 'Status', ENT_QUOTES, 'UTF-8') ?>
            </label>
            <select name="status" class="w-full p-2 border rounded">
              <option value="in_progress">
                <?= htmlspecialchars($langText['in_progress'] ?? 'In Progress', ENT_QUOTES, 'UTF-8') ?>
              </option>
  ...
  <div id="projectDetailsModal"
       class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white rounded-md p-8 w-90 max-h-[90vh] overflow-y-auto mt-10 relative">
      <button type="button" id="closeProjectDetailsModal"
      ...
      <form id="projectDetailsForm" action="<?= $baseUrl ?>/projects/update" method="POST">
        ...
        <div class="mb-4">
          ...
          <div class="flex mt-2">
            <input type="text" id="detailsNewTaskInput" class="w-full p-2 border rounded"
                   placeholder="<?= htmlspecialchars($langText['task_placeholder'] ?? 'Task description', ENT_QUOTES, 'UTF-8') ?>">
            <button type="button" id="detailsAddTaskBtn"
                    class="ml-2 bg-blue-500 text-white px-3 py-2 rounded">
              <?= htmlspecialchars($langText['add'] ?? 'Add', ENT_QUOTES, 'UTF-8') ?>
            </button>
          </div>
        ...
        <div class="mb-4">
          ...
          <div class="flex mt-2">
            ...
            <button type="button" id="detailsAddEmployeeBtn"
                    class="ml-2 bg-blue-500 text-white px-3 py-2 rounded">
              <?= htmlspecialchars($langText['add'] ?? 'Add', ENT_QUOTES, 'UTF-8') ?>
            </button>
          </div>
