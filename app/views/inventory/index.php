      ...
      <div id="projectSelectDiv" class="hidden mb-4">
        ...
        <select name="project_id" class="w-full p-2 border rounded">
          ...
          <?php foreach($activeProjects as $p): ?>
            <option value="<?= htmlspecialchars($p['id'], ENT_QUOTES) ?>"><?= htmlspecialchars($p['name'], ENT_QUOTES) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      ...
      <div id="newItemDiv" class="hidden mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="number" id="newItemQty" name="new_item_qty" min="1" class="w-full p-2 border rounded">
          </div>
        </div>
      </div>
      ...
      <div id="stockItemsDiv" class="mb-6">
        ...
        <?php else: foreach($allItems as $it): if($it['quantity']<=0) continue; ?>
          <div class="flex items-center mb-2">
            ...
            <span class="flex-1"><?= htmlspecialchars($it['name'], ENT_QUOTES) ?> (<?= (int)$it['quantity'] ?> disponíveis)</span>
            <input type="number"
...
<div id="inventoryHistoryModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
  <div class="bg-white rounded-md p-8 w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto relative">
    ...
    ?>
      <div class="history-item p-4 mb-2 bg-gray-50 rounded <?= $border ?>" data-id="<?= htmlspecialchars($m['id'], ENT_QUOTES) ?>">
        <div class="flex justify-between">
          <div>
            <span class="font-semibold"><?= htmlspecialchars($m['user_name'], ENT_QUOTES) ?></span>
            &mdash; <?= htmlspecialchars($m['datetime'], ENT_QUOTES) ?>
          </div>
          <span class="px-2 py-1 text-sm bg-white rounded"><?= ucfirst(htmlspecialchars($m['reason'], ENT_QUOTES)) ?></span>
        </div>
